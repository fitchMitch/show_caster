...
(function() {
  var max_char, message;

  max_char = 250;

  message = 'En attente ...';

  $('#announce_body').on('keyup', function() {
    var car_count, common;
    common = "caractères - " + max_char + " car max";
    car_count = this.value.length;
...
(function() {
  $(document).ready(function() {
    var course_title_change, switch_autocoached_course;
    course_title_change = function(e) {
      var cond, first_word, title;
      cond = $("input:checked").val() === '0';
      first_word = cond ? "Cours" : "Auto-coachée";
      title = first_word + " du " + ($("#course_event_date_3i").val()) + " ";
      title += "" + ($("#course_event_date_2i option:selected").text());
      return $("#course_title").val(title);
    };
    switch_autocoached_course = function(e) {
      var show_players;
    ...
    $("#course_is_autocoached_1, #course_is_autocoached_0").on('click', switch_autocoached_course);
    $("#course_event_date_3i").on('change', course_title_change);
    return $("#course_event_date_2i").on('change', course_title_change);
  });

}).call(this);
(function() {
  $(function() {
    var change_day_name, course_selectors, days, i, len, performance_selectors, sel, selectors_val, trigger, type, types;
    days = ["dimanche", "lundi", "mardi", "mercredi", "jeudi", "vendredi", "samedi"];
    types = ["course", "performance"];
    performance_selectors = (function() {
      var i, results;
      results = [];
      for (sel = i = 1; i <= 3; sel = ++i) {
        results.push("#performance_event_date_" + sel + "i");
      }
      return results;
    })();
    course_selectors = (function() {
      var i, results;
      results = [];
      for (sel = i = 1; i <= 3; sel = ++i) {
        results.push("#course_event_date_" + sel + "i");
      }
      return results;
    })();
    selectors_val = function(selector_array) {
      var i, len, results;
      results = [];
      for (i = 0, len = selector_array.length; i < len; i++) {
        sel = selector_array[i];
        results.push($(sel).val());
      }
      return results;
    };
    change_day_name = function(sels) {
      var d;
      d = new Date(selectors_val(sels).join(","));
      if ($(".d-flex.flex-row > #day_name").length) {
        $("#day_name").remove();
      }
    ...
    trigger = function(type) {
      ...
      if ($("#" + type + "_event_date_2i").length) {
        ...
        return $(document).on('change', selectors.join(","), function() {
          return change_day_name(selectors);
        });
      }
    };
...
(function() {
  ...
  if ($('.exercice-search').length > 0) {
    $('#q_skills_name_in, #q_energy_level_eq, #q_category_eq, #q_title_or_instructions_cont').on('change', function() {
      return $('.exercice-search').submit();
    });
  }
...
(function() {
  $(document).ready(function() {
    var allow_submission, indicatorsDisplay, observer, picture_handler, target, write_it;
    write_it = function(w, h) {
      var s;
      s = "hauteur : " + h + "  x largeur : " + w;
      $('#widthbox').text(s);
    };
    indicatorsDisplay = function(w, h) {
      if (w > 0) {
    ...
    allow_submission = function(e) {
      if (picture_handler === void 0 || picture_handler.files.length === 0) {
...
(function() {
  $('#answers').on('cocoon:before-insert', function(e, answer_to_be_added) {
    answer_to_be_added.fadeIn('slow');
  }).on('cocoon:before-remove', function(e, answering) {
...
(function() {
  var $element, max_char, message, random_time;
...
(function() {
  var remove_vote_classes, vote_validate;

  remove_vote_classes = function(selector) {
    selector.removeClass("validated_vote invalidated_vote mitigated_vote");
  };

  vote_validate = function(elem) {
    return elem.form.submit();
  };

  $("[name*='vote_label']").each(function(index) {
    $("option:selected", this).each(function(e) {
      var elder;
      console.log("val " + $(this).val());
      elder = $(this).parent();
      remove_vote_classes(elder);
      if ($(this).val() === 'yess') {
        elder.addClass("validated_vote");
      }
      if ($(this).val() === 'noway') {
        elder.addClass("invalidated_vote");
      }
      if ($(this).val() === 'maybe') {
        return elder.addClass("mitigated_vote");
      }
    });
  });

  $("[name*='vote_label']").on('change', function(e) {
