...
Commontator.configure do |config|
  ...
  # config.current_user_proc = lambda { |controller| controller.current_user || AnonymousUser.instance }
  config.current_user_proc = lambda { |controller|
  ...
  # }
  config.email_from_proc = ->(thread) { "no-reply@les-sesames.fr" }
  ...
  config.user_mentions_proc = ->(current_user, thread, query) {
    current_user.class.where('firstname LIKE ?', "#{query}%")
  }
end
